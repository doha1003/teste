...
function checkCriticalCriteria(qaResults) {
  ...
  const securityRate = qaResults.categories['보안 테스트']
    ? Math.round(
        (qaResults.categories['보안 테스트'].passed / qaResults.categories['보안 테스트'].total) *
          100
      )
    : 0;
  ...
  const accessibilityRate = qaResults.categories['접근성 테스트']
    ? Math.round(
        (qaResults.categories['접근성 테스트'].passed /
          qaResults.categories['접근성 테스트'].total) *
          100
      )
    : 0;
  ...
  const e2eRate = qaResults.categories['E2E 테스트']
    ? Math.round(
        (qaResults.categories['E2E 테스트'].passed / qaResults.categories['E2E 테스트'].total) * 100
      )
    : 0;
...
function checkRecommendedCriteria(qaResults) {
  ...
  const performanceRate = qaResults.categories['성능 테스트']
    ? Math.round(
        (qaResults.categories['성능 테스트'].passed / qaResults.categories['성능 테스트'].total) *
          100
      )
    : 0;
  ...
  const unitTestRate = qaResults.categories['단위 테스트']
    ? Math.round(
        (qaResults.categories['단위 테스트'].passed / qaResults.categories['단위 테스트'].total) *
          100
      )
    : 0;
  ...
  const codeQualityRate = qaResults.categories['코드 품질']
    ? Math.round(
        (qaResults.categories['코드 품질'].passed / qaResults.categories['코드 품질'].total) * 100
      )
    : 0;
...
function checkLighthouseScores() {
  ...
  try {
    ...
    if (fs.existsSync(lighthouseDir)) {
      ...
      const criteria =
        ENVIRONMENT_CRITERIA[environment]?.lighthouse || ENVIRONMENT_CRITERIA.production.lighthouse;
...
${warnings
  .filter((w) => w.severity !== 'lighthouse')
  .map(
    (warning) => `
...
${warnings
  .filter((w) => w.category === 'Lighthouse')
  .map(
    (warning) => `
