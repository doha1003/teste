...
function findHtmlFiles(dir = '.', files = []) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            if (['node_modules', 'dist', 'playwright-report', 'test-reports', 'design-system'].some(excluded => fullPath.includes(excluded))) {
                continue;
            }
        ...
        if (glob) {
            ...
            for (const pattern of htmlFilePatterns) {
                const foundFiles = glob.sync(pattern, { 
                    cwd: process.cwd(),
                    ignore: ['node_modules/**', 'dist/**', 'design-system/node_modules/**', 'playwright-report/**']
                });
