...
`;
  ...
  handleTouchMove(e) {
    ...
    // 스크롤이 감지되면 터치 피드백 제거
    if (deltaX > this.config.touch.scrollThreshold || deltaY > this.config.touch.scrollThreshold) {
  ...
  improveButtonTouchTargets() {
    ...
    buttons.forEach((button) => {
      ...
      if (rect.width < 44 || rect.height < 44) {
        ...
        button.style.padding = `${Math.max(parseInt(getComputedStyle(button).padding) || 0, 8)}px`;
      }
