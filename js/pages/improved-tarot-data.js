...
const detailedMajorArcana = {
    0: {
        ...
        upright: {
            ...
            general: '새로운 시작과 가능성이 가득한 시기입니다. 순수한 마음으로 도전해보세요.',
            love: '새로운 연애가 시작되거나 기존 관계에 새로운 활력이 생깁니다. 순수한 마음으로 사랑하세요.',
            career: '새로운 직장이나 사업을 시작하기 좋은 시기입니다. 두려워하지 말고 도전하세요.',
            money: '새로운 투자 기회가 있지만 신중하게 접근하세요. 무모한 투자는 피하는 것이 좋습니다.',
            health: '전반적으로 건강하고 활력이 넘칩니다. 새로운 운동이나 건강법을 시도해보세요.',
            advice: '과거에 얽매이지 말고 새로운 가능성을 믿고 도전해보세요. 실패를 두려워하지 마세요.'
        },
        reversed: {
            ...
            career: '준비 없는 이직이나 창업은 위험합니다. 더 신중하게 계획을 세우세요.',
            money: '충동적인 소비나 투자로 손실을 볼 수 있습니다. 계획적인 지출이 필요합니다.',
    ...
    1: {
        ...
        upright: {
            ...
            general: '원하는 것을 이룰 수 있는 능력과 의지가 있는 시기입니다. 집중해서 목표를 달성하세요.',
    ...
    2: {
        ...
        upright: {
            ...
            money: '투자에서 직감이 도움이 될 수 있지만, 너무 충동적이지는 말아야 합니다.',
...
const tarotHelpers = {
    ...
    analyzeCardCombination: function(cards) {
        const elements = cards.map(card => detailedMajorArcana[card.id]?.element || '');
    ...
    // 주요 원소 찾기
    getDominantElement: function(elements) {
        const count = {};
        elements.forEach(el => {
    ...
    // 원소별 에너지
    getElementEnergy: function(element) {
        const energies = {
            '공기': '변화와 소통의 에너지',
            '불': '열정과 행동의 에너지',
            '물': '감정과 직감의 에너지',
    ...
    // 조합 조언
    getCombinationAdvice: function(cards) {
        if (cards.length <= 1) return '단일 카드의 메시지에 집중하세요.';
    ...
    // 한국적 해석 추가
    addKoreanContext: function(cardInterpretation, question) {
        ...
        let context = '';
        for (let [key, keywords] of Object.entries(koreanKeywords)) {
...
const generateDetailedMessage = function(card, position, question) {
    ...
    if (!cardData) return '카드 정보를 찾을 수 없습니다.';
...
const monthlyTarotThemes = {
    1: { theme: '새로운 시작', cards: [0, 1, 19], message: '새해를 맞아 새로운 가능성을 탐색해보세요.' },
    2: { theme: '사랑과 관계', cards: [2, 6, 17], message: '사랑의 달, 인간관계에 집중하기 좋은 시기입니다.' },
    3: { theme: '성장과 발전', cards: [3, 8, 11], message: '봄의 시작과 함께 성장 에너지가 강한 시기입니다.' },
    4: { theme: '안정과 건설', cards: [4, 5, 15], message: '기반을 다지고 안정을 추구하기 좋은 시기입니다.' },
    5: { theme: '자유와 모험', cards: [0, 7, 14], message: '자유롭게 도전하고 새로운 경험을 쌓아보세요.' },
    6: { theme: '사랑과 결합', cards: [6, 2, 19], message: '사랑이 결실을 맺는 달, 관계 발전에 좋습니다.' },
    7: { theme: '열정과 성취', cars: [1, 8, 19], message: '뜨거운 여름처럼 열정적으로 목표를 추진하세요.' },
    8: { theme: '힘과 용기', cards: [8, 1, 7], message: '내면의 힘을 발휘하여 어려움을 극복하는 시기입니다.' },
    9: { theme: '지혜와 성찰', cards: [9, 12, 20], message: '가을처럼 성찰하고 지혜를 얻는 시기입니다.' },
    10: { theme: '변화와 운명', cards: [10, 13, 16], message: '큰 변화의 시기, 운명의 전환점에 있습니다.' },
    11: { theme: '균형과 조화', cards: [11, 14, 2], message: '균형을 찾고 조화로운 상태를 만들어가세요.' },
    12: { theme: '완성과 마무리', cards: [21, 20, 10], message: '한 해를 마무리하고 완성을 향해 나아가세요.' }
};
