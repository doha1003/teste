...
const sajuCalculator = {
    ...
    calculateTenGods: function(dayMaster, stems) {
        ...
        Object.keys(stems).forEach(position => {
            ...
            const isSameYinYang = this.getYinYang(this.stems.indexOf(dayMaster)) === 
    ...
    analyzeElements: function(characters) {
        ...
        const variance = Object.values(counts).reduce((sum, count) => sum + Math.pow(count - average, 2), 0) / 5;
    ...
    getTenGodCareer: function(tenGods) {
        ...
        const dominantGod = Object.keys(godCounts).reduce((a, b) => 
            godCounts[a] > godCounts[b] ? a : b, Object.keys(godCounts)[0]);
    ...
    interpretCompatibility: function(dayMaster, gender) {
        ...
        return `${dayMaster}일주인 당신과 잘 맞는 상대는 ${compatible.map(el => this.getElementStems(el)).flat().join(', ')}일주입니다. 
        특히 ${compatible[0]}의 기운을 가진 사람과는 서로를 보완하며 조화로운 관계를 만들 수 있습니다. 
        반면 ${avoid.map(el => this.getElementStems(el)).flat().join(', ')}일주와는 신중하게 접근하는 것이 좋습니다.
