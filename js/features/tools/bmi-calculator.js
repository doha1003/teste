    ...
    this.bmiRanges = [
      {
        ...
        color: '#3498db',
        description:
          '체중이 정상 범위보다 낮습니다. 균형 잡힌 식단과 적절한 운동으로 건강한 체중을 유지하세요.',
        advice: [
          '단백질이 풍부한 음식 섭취',
          '규칙적인 식사 시간 유지',
          '근력 운동으로 근육량 증가',
                ...
                <div class="bmi-chart">
                    <div class="bmi-chart-bar">
                        ${ranges
                          .map((range, index) => {
                            const width =
                              (range.max / maxBMI) * 100 -
                              (index > 0 ? (this.bmiRanges[index - 1].max / maxBMI) * 100 : 0);
...
// 전역 인스턴스 생성
export const bmiCalculatorService = new BMICalculatorService();

// 전역에도 연결 (레거시 코드 호환성)
window.bmiCalculatorService = bmiCalculatorService;
