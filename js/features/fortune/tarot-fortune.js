...
(function() {
    ...
    class TarotFortuneService extends FortuneService {
        constructor() {
            super({
            ...
            this.spreads = {
                ...
                celtic: { name: "켈틱 크로스", count: 10, positions: [
                    "현재 상황", "도전/십자가", "먼 과거", "가까운 과거", 
                    "가능한 미래", "가까운 미래", "자신", "외부 영향", 
                    "희망과 두려움", "최종 결과"
                ]}
            };
        ...
        updateSelectedCardsDisplay() {
            ...
            if (container) {
                container.innerHTML = this.selectedCards.map((card, idx) => `
        ...
        async performReading() {
            ...
            try {
                ...
                if (window.API_CONFIG && window.API_CONFIG.FORTUNE_API_URL) {
                    ...
                    if (response.ok) {
                        ...
                        if (result.success && result.data) {
                            const interpretation = this.parseTarotAIResponse(result.data);
                            this.showResult({
                ...
                console.warn('AI API 호출 실패, 기본 해석 사용');
            }
        ...
        parseTarotAIResponse(aiData) {
            ...
            return {
                interpretations: this.selectedCards.map((card, idx) => ({
                    position: this.currentSpread.positions[idx],
                    card: card,
                    interpretation: aiData.interpretations?.[idx] || this.generateCardInterpretation(card, this.currentSpread.positions[idx], card.isReversed ? card.meaning.reversed : card.meaning.upright)
                })),
