...
(function() {
    ...
    class ZodiacAnimalFortuneService extends FortuneService {
        constructor() {
            super({
        ...
        async fetchZodiacAnimalFortune(animal) {
            try {
                ...
                if (window.callGeminiAPI) {
                    const aiAnalysis = await this.generateAnimalFortuneWithGemini(animal, animalData);
                    if (aiAnalysis) {
                        this.showResult({
                            animal: animal,
                            animalData: animalData,
                ...
                console.error('띠별 운세 분석 오류:', error);
        ...
        generateFallbackFortune(animal, animalData) {
            ...
            const fortunes = {
                rat: {
                    overall: "쥐띠에게는 새로운 기회가 열리는 날입니다. 평소보다 더 적극적으로 행동하면 좋은 결과를 얻을 수 있습니다.",
                ...
                tiger: {
                    overall: "호랑이띠의 용기와 추진력이 돋보이는 날입니다. 과감한 도전이 성공으로 이어질 수 있습니다.",
            ...
            const luckyColors = ['빨간색', '파란색', '노란색', '초록색', '보라색', '주황색', '분홍색', '하늘색'];
