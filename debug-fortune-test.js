...
async function debugFortuneTest() {
  ...
  page.on('console', (msg) => {
    if (
      msg.type() === 'log' &&
      (msg.text().includes('API') || msg.text().includes('fortune') || msg.text().includes('운세'))
    ) {
  ...
  try {
    ...
    if (nameInput && yearSelect && monthSelect && daySelect && submitBtn) {
      ...
      if (resultDiv) {
        const isVisible = await resultDiv.evaluate(
          (el) =>
    ...
    if (zodiacNameInput && ariesRadio && zodiacSubmitBtn) {
      ...
      if (zodiacResultDiv) {
        const isZodiacVisible = await zodiacResultDiv.evaluate(
          (el) =>
            !el.classList.contains('d-none') &&
            !el.classList.contains('d-none-init') &&
            el.style.display !== 'none'
        ...
        console.log(
          `별자리 결과 텍스트 길이: ${zodiacResultText ? zodiacResultText.trim().length : 0}자`
        );
