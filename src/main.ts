...
import { ApplicationError } from '../types/global.js';
...
class DohaMainApp {
  ...
  public toggleMobileMenu(): void {
    ...
    const navMenu = document.querySelector<HTMLElement>('.nav-menu, .nav');
    const menuBtn = document.querySelector<HTMLButtonElement>('.mobile-menu-btn, .header__menu-toggle');
   ...
   * 서비스 탭 필터링
   */
  public showServices(category: string, _event?: Event): void {
   ...
   * 탭 버튼 상태 업데이트
   */
  private updateTabButtons(category: string): void {
    const buttons = document.querySelectorAll<HTMLButtonElement>('.tab-button, [role="tab"]');
   ...
   * 부드러운 스크롤
   */
  public smoothScroll(target: string): void {
    try {
  ...
  private throttle<T extends (...args: any[]) => any>(
    ...
    if (window.DohaApp?.utils?.throttle) {
      return window.DohaApp.utils.throttle(func, limit) as ThrottledFunction<T>;
    }
   ...
   * 결과 공유 기능
   */
  public shareResult(platform: string, title?: string, url?: string): void {
    try {
   ...
   * 알림 표시
   */
  public showNotification(message: string, type: 'success' | 'error' = 'success'): void {
    try {
   ...
   * 날짜 포맷팅
   */
  public formatDate(date: Date | string | number, format: string = 'YYYY-MM-DD'): string {
    try {
   ...
   * 숫자 포맷팅 (천 단위 콤마)
   */
  public formatNumber(num: number | string | null | undefined): string {
    try {
   ...
   * URL 파라미터 가져오기
   */
  public getUrlParam(name: string): Nullable<string> {
  ...
  private initLazyLoading(): void {
    try {
      const images = document.querySelectorAll<HTMLImageElement>('img[data-src]:not(.lazy)');
  ...
  private initDarkMode(): void {
    const isDarkMode = this.storage.get<boolean>('darkMode');
    if (isDarkMode) {
   ...
   * 운세 관련 함수들
   */
  public readonly fortuneHelpers = {
    ...
    load: <T = any>(type: string): Nullable<T> => {
      const today = this.formatDate(new Date(), 'YYYY-MM-DD');
      const fortuneKey = `fortune_${type}_${today}`;
      return this.storage.get<T>(fortuneKey);
    },
    
    share: (platform: string, fortuneType: string, _result: any): void => {
...
`;
   ...
   * 컴포넌트 로드 (ID 기반)
   */
  private async loadComponentById(componentName: string, targetId: string): Promise<void> {
    try {
   ...
   * 폴백 컴포넌트 로드
   */
  private loadFallbackComponent(componentName: string, targetId: string): void {
    const target = document.getElementById(targetId);
    if (!target) return;
   ...
   * AdSense 반응형 수정
   */
  private fixAdSenseResponsive(): void {
    const adContainers = document.querySelectorAll<HTMLElement>('.ad-container');
   ...
   * AdSense 초기화
   */
  public initAdSense(): void {
    try {
      this.fixAdSenseResponsive();
      
      const adSlots = document.querySelectorAll<HTMLElement>('.adsbygoogle:not([data-adsbygoogle-status])');
   ...
   * 데이터 시스템 로드
   */
  private async loadDataSystems(): Promise<void> {
    try {
   ...
   * 성능 최적화 시스템 로드
   */
  private async loadPerformanceOptimizer(): Promise<void> {
    try {
...
// 전역 함수로 내보내기 (기존 JavaScript와의 호환성)
declare global {
  interface Window {
    dohaApp: DohaMainApp;
