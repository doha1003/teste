...
class DeepFunctionalTest {
    ...
    async testMBTIFullFlow() {
            ...
            if (testLink) {
                ...
                const startBtn = await this.page.evaluate(() => {
                    ...
                    const startButton = buttons.find(btn => 
                        btn.innerText.includes('시작') || 
                        btn.innerText.includes('테스트')
                    );
                    if (startButton) {
                        startButton.click();
                        return true;
                ...
                if (startBtn) {
                    ...
                    const submitted = await this.page.evaluate(() => {
                        const submitBtns = document.querySelectorAll('button[type="submit"], .btn-submit, button.btn-primary');
        ...
        console.log(`   ${testResult.status === 'success' ? '✅' : '❌'} MBTI 테스트: ${testResult.status}`);
    }
    ...
    async testBMICalculator() {
        ...
        try {
            ...
            const heightFilled = await this.page.evaluate(() => {
                const inputs = ['#height', 'input[name="height"]', 'input[placeholder*="키"]', 'input[placeholder*="cm"]'];
            ...
            const weightFilled = await this.page.evaluate(() => {
                const inputs = ['#weight', 'input[name="weight"]', 'input[placeholder*="몸무게"]', 'input[placeholder*="kg"]'];
                for (const selector of inputs) {
                    const el = document.querySelector(selector);
                    if (el) {
                        el.value = '70';
                ...
                if (calculated) {
                    ...
                    const bmiValue = await this.page.evaluate(() => {
                        ...
                        for (const el of elements) {
                            ...
                            if (text && text.match(/\d+\.\d+/) && parseFloat(text.match(/\d+\.\d+/)[0]) > 10 && parseFloat(text.match(/\d+\.\d+/)[0]) < 50) {
                                return text;
                            }
                        }
                        return null;
    ...
    async testTextCounter() {
        ...
        try {
            ...
            if (textEntered) {
                ...
                const counts = await this.page.evaluate(() => {
                    ...
                    // 글자수
                    const charElements = document.querySelectorAll('[id*="char"], [class*="char"], .count-value');
        ...
        console.log(`   ${testResult.status === 'success' ? '✅' : '❌'} Text Counter: ${testResult.status}`);
    }
    ...
    async testDailyFortuneAPI() {
        ...
        try {
            ...
            await this.page.evaluate(() => {
                // 이름
                const nameInput = document.querySelector('#userName, input[name="userName"], input[placeholder*="이름"]');
            ...
            // 제출
            const submitted = await this.page.evaluate(() => {
                const submitBtn = document.querySelector('button[type="submit"], .btn-submit, button.btn-primary');
                if (submitBtn) {
                    submitBtn.click();
                    return true;
                }
            ...
            if (submitted) {
                ...
                if (response) {
                    ...
                    const fortuneText = await this.page.evaluate(() => {
                        const resultEl = document.querySelector('.fortune-result, .result-content, .fortune-text');
        ...
        console.log(`   ${testResult.status === 'success' ? '✅' : '❌'} Daily Fortune: ${testResult.status}`);
    }
        ...
        try {
            ...
            if (cardsSelected >= 3) {
                ...
                const readingStarted = await this.page.evaluate(() => {
                    const buttons = document.querySelectorAll('button');
                    for (const btn of buttons) {
                        if (btn.innerText.includes('리딩') || btn.innerText.includes('해석') || btn.innerText.includes('보기')) {
                            btn.click();
                            return true;
                        }
                    }
                ...
                if (readingStarted) {
                    ...
                    const readingText = await this.page.evaluate(() => {
                        const reading = document.querySelector('.reading-result, .tarot-result, .interpretation');
    ...
    generateReport() {
        ...
        console.log(`운세 서비스: ${report.summary.fortune.success}/${report.summary.fortune.total} 성공`);
