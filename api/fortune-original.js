...
`;
    ...
    // 타임아웃 설정 (13초, Vercel 함수 제한 고려 - 15초 설정에서 2초 마진)
    ...
    try {
      const result = await Promise.race([model.generateContent(aiPrompt), timeoutPromise]);
      ...
      if (aiError.message === 'AI 응답 타임아웃' || aiError.message.includes('fetch')) {
        ...
        // 기본 fallback 응답 제공
        const fallbackResponse = generateFallbackResponse(type, data);
...
function generateFallbackResponse(type, data) {
  ...
  if (type === 'daily') {
    return {
      ...
      descriptions: {
        overall:
          '오늘은 전반적으로 안정적인 하루가 될 것 같습니다. 큰 변화보다는 현재 상황을 유지하며 차근차근 진행하시기 바랍니다.',
        love: '인간관계에서 따뜻한 소통이 이루어질 것 같습니다. 가족이나 친구들과의 대화를 늘려보세요.',
        money: '금전적으로는 큰 지출을 피하고 저축에 집중하기 좋은 날입니다.',
        health: '규칙적인 생활 패턴을 유지하며 충분한 휴식을 취하시기 바랍니다.',
        work: '업무에 집중할 수 있는 좋은 날입니다. 계획한 일들을 차근차근 진행해보세요.',
      },
      luck: {
        time: '오후 2-4시',
        direction: '동쪽',
        color: '파란색',
        numbers: '7, 23',
        caution: '성급한 결정보다는 신중하게 생각해서 행동하시기 바랍니다.',
      },
    };
  } else if (type === 'zodiac' || type === 'zodiac-animal') {
    return {
      overall: '오늘은 차분하고 안정적인 하루가 될 것 같습니다.',
      scores: { love: 70, money: 75, work: 72, health: 78 },
      ...
      advice: '오늘은 무리하지 말고 여유를 갖고 행동하시기 바랍니다.',
      luckyNumber: '3, 17',
      luckyColor: '녹색',
    };
  } else {
    return '오늘은 전반적으로 안정적이고 평화로운 하루가 될 것 같습니다. 새로운 시작보다는 현재 상황을 정리하고 미래를 준비하는 시간으로 활용해보세요.';
  }
}
