...
async function handleTarotFortune(data, options, requestId) {
  ...
  switch (spread) {
    case 'single':
      ...
      return await tarotFortuneService.performSingleCardReading(
        cardNumbers[0],
        question,
        tarotOptions
      );
...
async function handleZodiacFortune(data, options, requestId) {
  ...
  if (!validation.valid) {
    throw new Error(
      `별자리 데이터 검증 실패: ${validation.errors.map((e) => e.message).join(', ')}`
    );
  }
...
async function compatibilityHandler(req, res, { requestId, clientIp, tracker }) {
  ...
  if (!type || !data) {
    return res
      .status(400)
      .json(createErrorResponse(400, getKoreanErrorMessage('validation_failed')));
  }
...
async function randomCardsHandler(req, res, { requestId, clientIp, tracker }) {
  ...
  if (isNaN(cardCount) || cardCount < 1 || cardCount > 10) {
    return res.status(400).json(createErrorResponse(400, '카드 수는 1-10 사이여야 합니다.'));
  }

  try {
    const excludeNumbers = Array.isArray(exclude)
      ? exclude.map((n) => parseInt(n)).filter((n) => !isNaN(n))
      : [];
...
async function healthHandler(req, res, { requestId, clientIp, tracker }) {
  try {
    ...
    const overallStatus = [dailyStatus, sajuStatus, tarotStatus, zodiacStatus, animalStatus].every(
...
// 명명된 내보내기 (테스트용)
export { fortuneHandler, compatibilityHandler, randomCardsHandler, healthHandler };
