#!/usr/bin/env python3
"""
테토-에겐 질문 최적화: 50개 → 20개 고품질 + 답변 선택지 다양화
"""

import json

def create_optimized_teto_questions():
    """20개 핵심 질문, 각 질문당 4-5개 선택지"""
    questions = [
        {
            "id": 1,
            "category": "social",
...
def create_updated_results():
    """8가지 결과를 4가지로 단순화하면서 품질 향상"""
    results = {
        "SUPER_TETO": {
            "type": "슈퍼 테토형",
            "emoji": "🔥",
...
if __name__ == "__main__":
    questions = create_optimized_teto_questions()
    results = create_updated_results()
    
    # JSON으로 저장
    with open('tests/teto-egen/optimized_questions_20.json', 'w', encoding='utf-8') as f:
        json.dump({
    ...
    print("테토-에겐 질문 최적화 완료!")
    print("50개 → 20개 질문으로 압축")
    print("각 질문당 4-5개 선택지로 디테일 향상")  
    print("8가지 → 4가지 결과 유형으로 단순화하면서 품질 향상")