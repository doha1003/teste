...
async function debugMBTIPage() {
  ...
  try {
        ...
        allDivs: Array.from(document.querySelectorAll('div'))
          ...
          .filter(
            (div) =>
              div.id ||
              div.classList.some(
                (cls) => cls.includes('mbti') || cls.includes('intro') || cls.includes('start')
              )
          ),
      };
    });
    ...
    debug.allButtons.forEach((btn, i) => {
      console.log(
        `  ${i + 1}. "${btn.textContent}" - 클래스: [${btn.classList.join(', ')}] - ID: ${btn.id} - Display: ${btn.style}`
      );
    });
    ...
    debug.allDivs.forEach((div, i) => {
      console.log(
        `  ${i + 1}. ID: ${div.id} - 클래스: [${div.classList.join(', ')}] - 텍스트: "${div.textContent}" - Display: ${div.style}`
      );
    });
    ...
    debug.allElements.slice(0, 20).forEach((el, i) => {
      console.log(
        `  ${i + 1}. <${el.tagName}> ID: ${el.id} - 클래스: [${el.classList.join(', ')}] - 텍스트: "${el.textContent}"`
      );
    });
