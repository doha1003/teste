...
class CSSPrefixImpactAnalysis {
  ...
  async scanAllFiles() {
    ...
    this.results.htmlFiles = await glob('**/*.html', {
      cwd: this.projectRoot,
      ignore: ['node_modules/**', '.backup/**', 'dist/**', 'test-results/**', 'playwright-report/**']
    });
    ...
    console.log(`📊 파일 스캔 완료: CSS ${this.results.cssFiles.length}개, HTML ${this.results.htmlFiles.length}개, JS ${this.results.jsFiles.length}개`);
  }
  ...
  async assessRisks() {
    ...
    for (const [className, usage] of this.results.classUsage) {
      const riskLevel = this.calculateRiskLevel(className, usage, highRiskPatterns, mediumRiskPatterns);
  ...
  calculateRiskLevel(className, usage, highRiskPatterns, mediumRiskPatterns) {
    ...
    // 사용 빈도 기반 위험도
    const totalFiles = [...new Set([...usage.cssFiles, ...usage.htmlFiles, ...usage.jsFiles])].length;
  ...
  getMethodName(content, index) {
    ...
    const methodMatch = beforeMatch.match(/(querySelector(?:All)?|getElementsByClassName|classList\\.(add|remove|contains|toggle))$/);
  ...
  async generateImpactReport() {
    ...
    const report = {
      ...
      summary: {
        ...
        mediumRiskClasses: this.results.riskAssessment.filter(r => r.riskLevel >= 3 && r.riskLevel < 5).length,
        lowRiskClasses: this.results.riskAssessment.filter(r => r.riskLevel < 3).length,
        totalFilesScanned: this.results.cssFiles.length + this.results.htmlFiles.length + this.results.jsFiles.length
      },
      fileCounts: {
    ...
    // JSON 리포트 저장
    const jsonReportPath = path.join(this.projectRoot, `css-prefix-impact-analysis-${Date.now()}.json`);
  ...
  generateMigrationPlan() {
    const plan = {
      phase1: {
        name: '긴급 대응 (즉시 실행)',
        classes: this.results.riskAssessment.filter(r => r.riskLevel >= 5).map(r => ({
          old: r.className,
          new: this.suggestNewClassName(r.className),
          files: [...new Set([...r.usage.cssFiles, ...r.usage.htmlFiles, ...r.usage.jsFiles])].length
        }))
      },
      phase2: {
        name: '우선 처리 (1주일 내)',
        classes: this.results.riskAssessment.filter(r => r.riskLevel >= 3 && r.riskLevel < 5).map(r => ({
          old: r.className,
          new: this.suggestNewClassName(r.className),
          files: [...new Set([...r.usage.cssFiles, ...r.usage.htmlFiles, ...r.usage.jsFiles])].length
        }))
      ...
      phase3: {
        name: '계획적 처리 (1개월 내)',
        classes: this.results.riskAssessment.filter(r => r.riskLevel < 3 && r.riskLevel >= 2).map(r => ({
          old: r.className,
          new: this.suggestNewClassName(r.className),
          files: [...new Set([...r.usage.cssFiles, ...r.usage.htmlFiles, ...r.usage.jsFiles])].length
        }))
  ...
  suggestNewClassName(oldClassName) {
    ...
    if (oldClassName.includes('btn') || oldClassName.includes('button')) return `dh-comp-${oldClassName}`;
    if (oldClassName.includes('form')) return `dh-comp-${oldClassName}`;
    if (oldClassName.includes('nav') || oldClassName.includes('menu')) return `dh-comp-${oldClassName}`;
    if (oldClassName.includes('text-')) return `dh-util-${oldClassName}`;
    if (oldClassName.includes('loading') || oldClassName.includes('spinner')) return `dh-state-${oldClassName}`;
...
${report.topConflicts.slice(0, 10).map(item => 
...
### Phase 1: 긴급 대응 (즉시 실행)
${report.migrationPlan.phase1.classes.map(cls => 
  `- \`${cls.old}\` → \`${cls.new}\` (${cls.files}개 파일)`
).join('\\n') || '해당 없음'}

### Phase 2: 우선 처리 (1주일 내)
${report.migrationPlan.phase2.classes.map(cls => 
  `- \`${cls.old}\` → \`${cls.new}\` (${cls.files}개 파일)`
).join('\\n') || '해당 없음'}

### Phase 3: 계획적 처리 (1개월 내)
${report.migrationPlan.phase3.classes.map(cls => 
  `- \`${cls.old}\` → \`${cls.new}\` (${cls.files}개 파일)`
).join('\\n') || '해당 없음'}
...
`;
  ...
  printSummary(report) {
    ...
    report.topConflicts.slice(0, 10).forEach((item, index) => {
      const totalFiles = [...new Set([...item.usage.cssFiles, ...item.usage.htmlFiles, ...item.usage.jsFiles])].length;
      console.log(`${index + 1}. ${item.className} (위험도: ${item.riskLevel}, 파일: ${totalFiles}개)`);
    });
