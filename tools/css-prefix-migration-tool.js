...
class CSSPrefixMigrationTool {
  ...
  async scanFiles() {
    ...
    console.log(
      `📊 스캔 완료: CSS ${this.cssFiles.length}개, HTML ${this.htmlFiles.length}개, JS ${this.jsFiles.length}개`
    );
  }
    ...
    for (const htmlFile of this.htmlFiles) {
      ...
      for (const [oldClass, newClass] of Object.entries(this.prefixMapping)) {
        ...
        const classPattern = new RegExp(
          `(class=["'][^"']*\\s?)${this.escapeRegex(oldClass)}(\\s[^"']*["']|["'])`,
          'g'
        );
  ...
  async updateJavaScriptFiles() {
    ...
    for (const jsFile of this.jsFiles) {
      ...
      for (const [oldClass, newClass] of Object.entries(this.prefixMapping)) {
        ...
        const patterns = [
          new RegExp(
            `(querySelector(?:All)?\\s*\\(\\s*['"\`][^'"\`]*\\.)${this.escapeRegex(oldClass)}([^'"\`]*['"\`]\\s*\\))`,
            'g'
          ),
          new RegExp(
            `(getElementsByClassName\\s*\\(\\s*['"\`])${this.escapeRegex(oldClass)}(['"\`]\\s*\\))`,
            'g'
          ),
          new RegExp(
            `(classList\\.(?:add|remove|contains|toggle)\\s*\\(\\s*['"\`])${this.escapeRegex(oldClass)}(['"\`]\\s*\\))`,
            'g'
          ),
        ];
  ...
  async generateReport() {
    ...
    const reportPath = path.join(
      this.projectRoot,
      `css-prefix-migration-report-${Date.now()}.json`
    );
    ...
    const summaryPath = path.join(
      this.projectRoot,
      `css-prefix-migration-summary-${Date.now()}.md`
    );
...
${Object.entries(report.prefixMapping)
