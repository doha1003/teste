...
async function quickFortuneTest() {
    ...
    page.on('response', async response => {
        if (response.url().includes('api/fortune') || response.url().includes('doha-kr-ap.vercel.app')) {
    ...
    if (results.daily && results.zodiac && results.apiWorking) {
        ...
        console.log('❌ API 연결에 문제가 있습니다. https://doha-kr-ap.vercel.app/api/fortune 을 확인해주세요.');
    }
