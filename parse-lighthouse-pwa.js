...
try {
  ...
  if (report.audits['installable-manifest']) {
    console.log('📱 PWA 설치 가능성:', report.audits['installable-manifest'].score === 1 ? '✅ 가능' : '❌ 불가능');
  ...
  if (report.audits['works-offline']) {
    console.log('🔌 오프라인 동작:', report.audits['works-offline'].score === 1 ? '✅ 가능' : '❌ 불가능');
  }
  
  if (report.audits['maskable-icon']) {
    console.log('🎭 Maskable 아이콘:', report.audits['maskable-icon'].score === 1 ? '✅ 존재' : '❌ 없음');
  }
