...
// JSON 파일 읽기
const data = JSON.parse(fs.readFileSync('test-results-comprehensive/comprehensive-report.json', 'utf8'));
...
Object.entries(errorCounts)
    ...
    .forEach(([error, info], index) => {
        ...
        console.log(`   발생 페이지: ${info.pages.slice(0, 3).join(', ')}${info.pages.length > 3 ? ' 외 ' + (info.pages.length - 3) + '개' : ''}`);
        console.log();
    });
