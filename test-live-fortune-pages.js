...
async function testFortunePage(browser, page, pageConfig) {
  ...
  try {
    ...
    const formExists = await page
      .$eval(pageConfig.testConfig.formSelector, () => true)
      .catch(() => false);
    if (formExists) {
...
async function testDailyFortune(page, result) {
  try {
    ...
    const resultElement = await page.$(
      '.fortune-result, .result-container, #result, .daily-result'
    );
    if (resultElement) {
...
async function testSajuFortune(page, result) {
  try {
    ...
    const timeInput = await page.$(
      'input[name="birth-time"], #birth-time, select[name="birth-time"]'
    );
    if (timeInput) {
...
async function testTarotFortune(page, result) {
  try {
    ...
    const resultElement = await page.$(
      '.fortune-result, .result-container, #result, .tarot-result'
    );
    if (resultElement) {
      const resultText = await resultElement.evaluate((el) => el.innerText);
      if (resultText && resultText.trim().length > 10) {
...
async function testZodiacFortune(page, result) {
  try {
    ...
    const resultElement = await page.$(
      '.fortune-result, .result-container, #result, .zodiac-result'
    );
    if (resultElement) {
      const resultText = await resultElement.evaluate((el) => el.innerText);
      if (resultText && resultText.trim().length > 10) {
...
async function testZodiacAnimalFortune(page, result) {
  try {
    ...
    const resultElement = await page.$(
      '.fortune-result, .result-container, #result, .animal-result'
    );
    if (resultElement) {
      const resultText = await resultElement.evaluate((el) => el.innerText);
      if (resultText && resultText.trim().length > 10) {
