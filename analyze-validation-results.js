...
class ValidationResultAnalyzer {
  constructor() {
    this.jsonPath = path.join(process.cwd(), 'test-results-comprehensive', 'comprehensive-report.json');
  }
  ...
  async analyze() {
    try {
      ...
      console.log(`핵심 페이지 성공률: ${Math.round((summary.criticalPages.success / summary.criticalPages.total) * 100)}%`);
      ...
      console.log(`가장 빠른 페이지: ${fastestPage?.name} (${fastestPage?.performance.loadTime}ms)`);
      console.log(`가장 느린 페이지: ${slowestPage?.name} (${slowestPage?.performance.loadTime}ms)`);
      ...
      console.log('\n📋 상세 보고서:');
      console.log(`HTML 리포트: ${path.join(process.cwd(), 'test-results-comprehensive', 'comprehensive-report.html')}`);
      console.log(`JSON 데이터: ${this.jsonPath}`);
      console.log(`스크린샷: ${path.join(process.cwd(), 'test-results-comprehensive', 'screenshots')}`);
