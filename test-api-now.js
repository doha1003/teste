...
function testHealthAPI() {
  return new Promise((resolve, reject) => {
    ...
    https
      .get('https://doha-kr-ap.vercel.app/api/health', (res) => {
        let data = '';
...
async function runAllTests() {
  ...
  try {
    ...
    console.log(
      `- Health API: ${healthResult.status === 200 ? '✅ 성공' : `❌ 실패 (${healthResult.status})`}`
    );
    console.log(
      `- Fortune API: ${fortuneResult.status === 200 ? '✅ 성공' : `❌ 실패 (${fortuneResult.status})`}`
    );
    console.log(
      `- Manseryeok API: ${manseryeokResult.status === 200 ? '✅ 성공' : `❌ 실패 (${manseryeokResult.status})`}`
    );
