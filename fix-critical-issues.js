...
class CriticalIssuesFixer {
  ...
  fixPretendardFont() {
    ...
    const files = ['css/core/typography.css', 'css/korean-optimization.css', 'includes/head.html'];
  ...
  fixCSPPolicy() {
    ...
    if (fs.existsSync(vercelConfig)) {
      ...
      const cspHeader = config.headers.find(
        (h) => h.headers && h.headers.find((hh) => hh.key === 'Content-Security-Policy')
      );
  ...
  fixTextCounter() {
    ...
    updateCounts(text) {
      ...
      if (content.includes('updateCounts')) {
        content = content.replace(/updateCounts\s*\([^)]*\)\s*{[^}]*}/, improvedUpdate);
      } else {
        // 함수가 없으면 추가
        content = content.replace(
...
${improvedUpdate}`
  ...
  fixMBTIAdPopup() {
    ...
    if (fs.existsSync(file)) {
      ...
      // IQ 테스트 광고 제거
      content = content.replace(/<div[^>]*class="[^"]*ad[^"]*"[^>]*>.*?<\/div>/gis, '');
  ...
  fixTarotCardSelection() {
    ...
    <div id="tarot-cards-container" class="tarot-cards-grid">
        ...
        content = content.replace('<!-- 카드 선택 영역 -->', cardGrid);
      }
