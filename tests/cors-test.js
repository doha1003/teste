...
const testCases = [
    ...
    // 개발 환경에서만 허용
    { origin: 'http://localhost:5173', expected: process.env.NODE_ENV === 'development', description: 'Vite 개발 서버' },
    { origin: 'http://localhost:3000', expected: process.env.NODE_ENV === 'development', description: '일반 개발 서버' },
    { origin: 'http://127.0.0.1:5173', expected: process.env.NODE_ENV === 'development', description: 'IP 주소 개발 서버' },
...
testCases.forEach((testCase, index) => {
    ...
    if (success) {
        ...
        console.log(`   예상: ${testCase.expected ? '허용' : '차단'}, 실제: ${result ? '허용' : '차단'}`);
        failed++;
    }
});
