...
class FortuneVerificationTest {
    ...
    async testZodiacFortune() {
        ...
        try {
            ...
            // 결과 내용 확인
            const fortuneText = await page.$eval('#fortuneResult, .fortune-content', el => el.textContent.trim());
            if (fortuneText.length > 10) {
    ...
    async testZodiacAnimalFortune() {
        ...
        try {
            ...
            // 호랑이띠 선택 (또는 첫 번째 띠)
            await page.click('.animal-card[data-animal="tiger"], .zodiac-animal-card[data-animal="tiger"], .animal-card:first-child, .zodiac-animal-card:first-child');
    ...
    generateReport() {
        ...
        this.results.forEach((result, index) => {
            ...
            if (result.apiCalls.length > 0) {
                ...
                result.apiCalls.forEach(call => {
                    if (call.status) {
                        console.log(`     - ${call.method || 'GET'} ${call.url} → ${call.status} ${call.statusText}`);
                    } else if (call.method) {
        ...
        console.log('\n' + '='.repeat(60));
        console.log(`📊 전체 결과: ${successCount}/${totalTests} 성공 (${Math.round(successCount/totalTests*100)}%)`);
