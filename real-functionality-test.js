...
async function testRealFunctionality() {
    ...
    try {
        ...
        await page.goto('http://localhost:3000/tests/love-dna/test.html', { waitUntil: 'networkidle0' });
        
        // 기본적인 페이지 로드 확인
        const loveDnaLoaded = await page.$('.test-container') || await page.$('.question-container');
        if (loveDnaLoaded) {
        ...
        await page.goto('http://localhost:3000/tests/teto-egen/test.html', { waitUntil: 'networkidle0' });
        
        const tetoEgenLoaded = await page.$('.test-container') || await page.$('.question-container');
        if (tetoEgenLoaded) {
        ...
        await page.goto('http://localhost:3000/tools/bmi-calculator.html', { waitUntil: 'networkidle0' });
